...
Gets a baseline cohort for oncologists billing to CMS (part B & fee-for-service) in 2017.
...
phys_import <- data.table::fread("Medicare_Physician_Other_Practitioners_by_Provider_2017.csv")
...
QUERY_temp <- dbSendQuery(con, "CREATE TABLE physicians_onco AS SELECT * 
                          FROM physicians2017 WHERE LOWER(Rndrng_Prvdr_Type) LIKE '%onco%'")
...
QCs the R & SQL code outputs match
...
This section separates out the 2 types of payments, to get a merged list of each physician and the record of payment associated with it. 
